...
<div>
    ...
    <div class="modal-body">
        ...
        @Using (Html.BeginForm("Search", "Shippings", Nothing, FormMethod.Post, New With {.enctype = "multipart/form-data"}))
        ...
        <div id="SearchResult">
            <table Class="table">
                ...
                @For Each item In Model.Items
                    @Using (Html.BeginForm("AddItem", "Shippings", Nothing, FormMethod.Post))
                        @<tr>
                            <td>
                            ...
                            <td>
                                @*商品がカート（出荷明細テーブル）に入っていなければ、「追加」ボタンを表示する*@
    ...
    <div Class="modal-footer">
        @If Not ViewBag.shippingdetailsRowCount = 0 Then
            @Html.ActionLink("出荷商品一覧", "Create", New With {.controller = "Shippings"}, New With {.class = "btn btn-default", .type = "button"})
        End If
        @Html.ActionLink("在庫一覧へ戻る", "Cancel", New With {.controller = "Shippings"}, New With {.class = "btn btn-default", .type = "button", .id = "deleteConfirm"})
    </div>
</div>

@Section Scripts
    <script src="~/Scripts/Shippings/search.js"></script>
End Section